...
public class JdbcUserDao implements UserDao {
    ...
    public User getUserByUsername(String username) {
        ...
        String sqlSearchForUser = "SELECT user_id, username FROM users WHERE UPPER(username) = ?;";
    ...
    public Map<String, String> getPasswordAndSaltByUsername(String username) {
        ...
        String sqlSearchForUser = "SELECT salt, password_hash FROM users WHERE UPPER(username) = ?;";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sqlSearchForUser, username.toUpperCase());
        
        if (results.next()) {
            passwordAndSalt.put("salt", results.getString("salt"));
            passwordAndSalt.put("password_hash", results.getString("password_hash"));
        }
